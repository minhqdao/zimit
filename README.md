...
- **Bounded key storage:** Configure `max_entries` and `idle_timeout_ns` to reclaim inactive, fully-drained keys. New keys return `error.CapacityExceeded` when no safe entry can be reclaimed.
